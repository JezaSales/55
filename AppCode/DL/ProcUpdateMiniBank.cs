    ...
    public class ProcMiniBankGetDBStatus : IProcedure
    {
        ...
        public object Call(object obj)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    res.CommonStr= dt.Rows[0]["_APICode"] is DBNull ? string.Empty : dt.Rows[0]["_APICode"].ToString();
                    res.CommonStr2= dt.Rows[0]["_SPKey"] is DBNull ? string.Empty : dt.Rows[0]["_SPKey"].ToString();
                    res.CommonInt= dt.Rows[0]["_Status"] is DBNull ?0 : Convert.ToInt32(dt.Rows[0]["_Status"]);
                }
            }
            catch (Exception ex)
            {
        ...
        public string GetName() => "declare @APIID int,@SPKey varchar(10),@Status tinyint;select @APIID=_APIID,@SPKey=_SPKey,@Status=_Type from tbl_Transaction(nolock) where _TID=@TID;select _APICode,@SPKey _SPKey,@Status _Status from tbl_API where _ID =@APIID;";
    }
}
