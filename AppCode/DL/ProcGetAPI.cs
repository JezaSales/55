                    ...
                    if (req.IsListType)
                    {
                        foreach (DataRow row in dt.Rows)
                        {
                            var aPIDetail = new APIDetail
                            {
                                ...
                                BillContentType = row["_BillFetchContentType"] is DBNull ? 0 : Convert.ToInt16(row["_BillFetchContentType"])
                            };
                            resp.Add(aPIDetail);
                        }
                    }
                    else
                    {
                        if (dt.Rows.Count > 0)
                        {
                            return new APIDetail
                            {
                                ...
                                BillContentType = dt.Rows[0]["_BillFetchContentType"] is DBNull ? 0 : Convert.ToInt16(dt.Rows[0]["_BillFetchContentType"])
