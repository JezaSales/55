                ...
                if (dt.Rows.Count > 0)
                {
                    if (req.CommonInt == -1)
                    {
                        foreach (DataRow row in dt.Rows)
                        {
                            var membershipMaster = new MembershipMaster
                            {
                                ...
                                IsTarget = row["_IsTarget"] is DBNull ? false : Convert.ToBoolean(row["_IsTarget"]),
                    ...
                    else
                    {
                        return new MembershipMaster
                        {
                            ...
                            IsTarget = dt.Rows[0]["_IsTarget"] is DBNull ? false : Convert.ToBoolean(dt.Rows[0]["_IsTarget"]),
