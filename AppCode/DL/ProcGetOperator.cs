                ...
                if (dt.Rows.Count > 0)
                {
                    if (req.CommonInt == 0 || req.CommonInt2 > 0)
                    {
                        foreach (DataRow row in dt.Rows)
                        {
                            var operatorDetail = new OperatorDetail
                            {
                                ...
                                IsPaidAdditional = row["_IsPaidAdditional"] is DBNull ? false : Convert.ToBoolean(row["_IsPaidAdditional"]),
                                IsAdditionalServiceType = row["_IsAdditionalServiceType"] is DBNull ? false : Convert.ToBoolean(row["_IsAdditionalServiceType"])
                            };
                    ...
                    else
                    {
                        return new OperatorDetail
                        {
                            ...
                            IsPaidAdditional = dt.Rows[0]["_IsPaidAdditional"] is DBNull ? false : Convert.ToBoolean(dt.Rows[0]["_IsPaidAdditional"]),
                            IsAdditionalServiceType = dt.Rows[0]["_IsAdditionalServiceType"] is DBNull ? false : Convert.ToBoolean(dt.Rows[0]["_IsAdditionalServiceType"])
                        };
                    }
                }
            }
