...
namespace RoundpayFinTech.AppCode.DL.Shopping
{
    public class ProcProceedToPay : IProcedure
    {
            ...
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        res.PrimaryDeductionPer = dt.Rows[0]["PrimaryDeductionPer"] is DBNull ? 0 : Convert.ToInt32(dt.Rows[0]["PrimaryDeductionPer"]);
                        res.SecondaryDeductionPer = dt.Rows[0]["SecondaryDeductionPer"] is DBNull ? 0 : Convert.ToInt32(dt.Rows[0]["SecondaryDeductionPer"]);
                        ...
                        res.TotalOAmount = dt.Rows[0]["TotalOAmount"] is DBNull ? 0 : Convert.ToDecimal(dt.Rows[0]["TotalOAmount"]);
                        ...
                        res.UPBalance = dt.Rows[0]["UPBalance"] is DBNull ? 0 : Convert.ToDecimal(dt.Rows[0]["UPBalance"]);
                        res.USBalance = dt.Rows[0]["USBalance"] is DBNull ? 0 : Convert.ToDecimal(dt.Rows[0]["USBalance"]);
                        res.SDeduction = dt.Rows[0]["SDeduction"] is DBNull ? 0 : Convert.ToDecimal(dt.Rows[0]["SDeduction"]);
                        res.secWalletStatus = dt.Rows[0]["secWalletStatus"] is DBNull ? false : Convert.ToBoolean(dt.Rows[0]["secWalletStatus"]);
        ...
        public string GetName() => "Proc_ProceedToPay_New";
    }
}