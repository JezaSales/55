            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (_res.Statuscode == ErrorCodes.One)
                    {
                        ...
                        _res.IRCTCStaus = dt.Rows[0]["_IRCTCStaus"] is DBNull ? 0 : Convert.ToInt16(dt.Rows[0]["_IRCTCStaus"]);
                        _res.IRCTCID = dt.Rows[0]["_IRCTCID"] is DBNull ? string.Empty : dt.Rows[0]["_IRCTCID"].ToString();
                        _res.IRCTCExpiry = dt.Rows[0]["_IRCTCExpiry"] is DBNull ? string.Empty : dt.Rows[0]["_IRCTCExpiry"].ToString();
                        _res.RequestID = dt.Rows[0]["_RequestID"] is DBNull ? string.Empty : dt.Rows[0]["_RequestID"].ToString();
                    }
                }
            }
            catch (Exception ex)
