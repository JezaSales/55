...
{
    public class ProcGetAfItemsLinks : IProcedure
    {
        private readonly IDAL _dal;
        public ProcGetAfItemsLinks(IDAL dal) => _dal = dal;
                        ...
                        if (Convert.ToInt32(dt.Rows[0][0]) == 1)
                        {
                            foreach (DataRow dr in dt.Rows)
                            {
                                var data = new AffiliatedItem
                                {
                                    ...
                                    Link = Convert.ToString(dr["_Link"]).Replace("{USERID}", req.LoginID.ToString()).Replace("{TID}", Convert.ToString(dr["TID"])),
        ...
        public string GetName() => "proc_GetAfItemsLinks";
    }
}
