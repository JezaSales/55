            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (_res.Statuscode == ErrorCodes.One)
                    {
                        ...
                        _res.StateName = dt.Rows[0]["_StateName"] is DBNull ? string.Empty : dt.Rows[0]["_StateName"].ToString();
                        _res.Email = dt.Rows[0]["_Email"] is DBNull ? string.Empty : dt.Rows[0]["_Email"].ToString();
                        _res.Address = dt.Rows[0]["_Address"] is DBNull ? string.Empty : dt.Rows[0]["_Address"].ToString();
                        _res.BMobile = dt.Rows[0]["_MobileNo"] is DBNull ? string.Empty : dt.Rows[0]["_MobileNo"].ToString();
                    }
                }
            }
            catch (Exception ex)
