                ...
                if (dt.Rows.Count > 0)
                {
                        ...
                        if (Convert.ToInt32(dt.Rows[0][0]) == 1)
                        {
                            foreach (DataRow dr in dt.Rows)
                            {
                                var data = new AffiliatedItem
                                {
                                    ...
                                    Tittle = Convert.ToString(dr["_Tittle"]),
                    ...
                    else
                    {
                        if (Convert.ToInt32(dt.Rows[0][0]) == 1)
                        {
                            ...
                            response.Name = Convert.ToString(dt.Rows[0]["_Name"]);
                            response.OID = Convert.ToInt32(dt.Rows[0]["_OID"]);
                            response.IsImageURL = Convert.ToBoolean(dt.Rows[0]["_IsImageURL"]);
                            response.Tittle = Convert.ToString(dt.Rows[0]["_Tittle"]);
                            response.Description = Convert.ToString(dt.Rows[0]["_Description"]);
                        }
                    }
                }
            }
