...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class SlabML : ISlabML
    {
        ...
        public SlabDetailModel GetB2cSlabDetail(int SlabID, int OpTypeID)
        {
            ...
            if (_lr.LoginTypeID == LoginType.ApplicationUser || (userML.IsCustomerCareAuthorised(ActionCodes.AddEditSLAB)))
            {
                ...
                IProcedure proc = new ProcGetB2CSlabDetail(_dal);
        ...
        public IResponseStatus UpdateSlabDetail(SlabCommission slabCommission, string slabType="G")
        {
        ...
        public Serviceroute BBPSServiceRoute()
        {
            var Req = new CommonReq
            {
                UserID = _lr.UserID
            };
            IProcedure _proc = new ProcGetUserServiceRoute(_dal);
            return (Serviceroute)_proc.Call(Req);
        }
        public ResponseStatus BBPSServiceRouteSave(int sr, int srp)
        {
            var _resp = new ResponseStatus
            {
                Statuscode = ErrorCodes.Minus1,
            ...
            IProcedure _proc = new ProcUpdateServiceRoute(_dal);
            _resp = (ResponseStatus)_proc.Call(Req);
            return _resp;
        }
        ...
        public async Task<List<SlabdetailServicewise>> GetSlabdetailServicewise(int slabId)
        {
            IProcedureAsync _proc = new ProcGetSlabdetailServicewise(_dal);
            var resp = (List<SlabdetailServicewise>)await _proc.Call(slabId).ConfigureAwait(false);
            return resp ?? new List<SlabdetailServicewise>();
        }

        public IResponseStatus B2CdislaycommissionUpdate(SlabdetailServicewise AddData)
        {
            ...
            IProcedure _proc = new ProcUpdateB2Cdislaycommission(_dal);
