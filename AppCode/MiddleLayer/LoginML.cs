...
namespace Fintech.AppCode
{
    public class LoginML : ILoginML
    {
        ...
        public WebsiteInfo GetWebsitePackage(int ID, bool IsB2C=false)
        {
            if (ID > 0)
            {
                IProcedure _proc = new ProcGetWebsitePackage(_dal);
                return (WebsiteInfo)_proc.Call(new CommonReq { CommonInt = ID, CommonBool = IsB2C });
            }
            return new WebsiteInfo();
        }
        ...
        public AppLoginResponse DoAppLogin(LoginDetail loginDetail)
        {
            ...
            responseStatus.IsShowUPI = _lr.UPIRequestModeID.In(AllowUpi.Both,AllowUpi.FromApp);
            return responseStatus;
        }
            ...
            if (_lr.LoginTypeID == LoginType.ApplicationUser)
            {
                ...
                else if (_lr.RoleID == Role.Retailor_Seller)
                {
                    ...
                    //if (ApplicationSetting.DynamicWebsiteType == Fintech.AppCode.StaticModel.DynamicWebsiteType.Disabled)
                        if (ApplicationSetting.DynamicWebsiteType == DynamicWebsiteType.Shopping && _lr.LaunchPreferences == LaunchPreferences.DynamicWebSiteIndex)
                        {
                            return "Home";
                        }
                        else
