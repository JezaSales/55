...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class WhatsappML : IWhatsappML
    {
        ...
        public async Task<WhatsappConversation> SendWhatsappSessionMessageAllAPI(WhatsappConversation wc, bool Isautoreply = false)
        {
            ...
            try
            {
                ...
                if (res.Statuscode == 1)
                {
                    ...
                    else if (wc.APICODE == WhatsappAPICode.THREESIXTY)
                    {
                        //Whatsapp THREESIXTY Api  Call
                        response = await WhatsappThreeSixty(wc, res);
                    }

                    else if (wc.APICODE == WhatsappAPICode.ALERTHUB)
                    {
        ...
        private async Task<SendSessionMessageResponse> WhatsappThreeSixty(WhatsappConversation wc, WhatsappConversation res)
        {
            //Whatsapp Watteam Api Text and MEdia Send  In a Api
            var wAPIML = new WhatsappAPIML(_accessor, _env, _dal);
            var objWATEAM = new WhatsappAPIWaTeam
            ...
            return await wAPIML.ThreeSixtySendSessionMessage(objWATEAM);
        }
        ...
        public ResponseStatus CreateWhatsappLead(int ContactID)
        {
            var req = new CommonReq()
            {
                LoginTypeID = _lr.LoginTypeID,
            ...
            IProcedure _proc = new ProcCreateWhatsappLead(_dal);
            var res = (ResponseStatus)_proc.Call(req);
            return res;
        }
