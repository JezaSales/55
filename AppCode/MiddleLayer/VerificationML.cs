...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class VerificationML : IVerificationML
    {
        ...
        public VerificationOutput UPIVerification(VerificationInput verificationInput)
        {
            ...
            {
                IVerificationAPI verificationAPI = new ThirdParty.Hypto.HyptoML(_accessor, _env, _dal);
            ...
            if (serviceRes.APICode.Equals(APICode.RAZORPAYOUT) || serviceRes.APICode.EndsWith("RZRPOT"))
            {
               var moneyTransferAPIML = new RZRPayoutML(_accessor, _env, _dal, serviceRes.APICode);
                var APIRes = moneyTransferAPIML.DoUPIVerification(new MTAPIRequest {
                    TransactionID = serviceRes.TransactionID,
                    TID = serviceRes.TID,
                    mBeneDetail = new MBeneDetail
                    {
            ...
            if (serviceRes.APICode.Equals(APICode.CASHFREE))
            {
                var moneyTransferAPIML = new CashFreePayoutML(_accessor, _env, _dal, string.Empty);
                var APIRes = moneyTransferAPIML.DoUPIVerification(new MTAPIRequest
                {
                    TransactionID = serviceRes.TransactionID,
                    TID = serviceRes.TID,
            ...
            if (serviceRes.APICode.Equals(APICode.DECENTRO))
            {
                var moneyTransferAPIML = new DecentroML(_accessor, _env, _dal, APICode.DECENTRO);
                var APIRes = moneyTransferAPIML.DoUPIVerification(new MTAPIRequest
                {
                    TransactionID = serviceRes.TransactionID,
                    TID = serviceRes.TID,
                    APIOpCode = serviceRes.APIOpCode,
                    mBeneDetail = new MBeneDetail
                    {
                        AccountNo = verificationInput.AccountNo
                    }
