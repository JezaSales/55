    ...
    public class OperatorML : IOperatorML, IOperatorAppML, ITargetML
    {
        ...
        public IEnumerable<IndustryTypeModel> GetIndustryWiseOpTypeList()
        {
            ...
            if (Procres.Count > 0)
            {
                ...
                var distinctIndustry = Procres.GroupBy(d => new { d.ID, d.IndustryType })
         .Select(m => new IndustryTypeModel { ID = m.Key.ID, IndustryType = m.Key.IndustryType }).ToList();
        ...
        public IEnumerable<OpTypeMaster> GeB2CtOptypeInSlab()
        {
            ProcGetOperator proc = new ProcGetOperator(_dal);
            return proc.OpTypesInSlab();
        }
        ...
        public async Task<IEnumerable<string>> GetDowns()
        {
            ...
            return (List<string>)await proc.Call(new CommonReq { LoginTypeID = _lr.LoginTypeID, LoginID = _lr.UserID }).ConfigureAwait(false);
        }
        ...
        public IResponseStatus SaveTargetB2c(TargetModel rangeDetail)
        {
            var res = new ResponseStatus
            {
                Statuscode = ErrorCodes.Minus1,
            ...
            if (_lr.LoginTypeID == LoginType.ApplicationUser && _lr.RoleID == Role.Admin || userML.IsCustomerCareAuthorised(ActionCodes.AddEditOPERATOR))
            {
                ...
                IProcedure proc = new ProcSaveTargetB2c(_dal);
                return (IResponseStatus)proc.Call(rangeModelReq);
            }
            return res;
        }
        ...
        public TargetModel GetTargetB2c(TargetModel DetailReq)
        {
            var res = new TargetModel();
            if (!_lr.RoleID.In(Role.APIUser, Role.Retailor_Seller))
            {
                ...
                IProcedure proc = new ProcGetTargetB2c(_dal);
                res = (TargetModel)proc.Call(Request);
            }
            return res;
        }
        ...
        public IEnumerable<ServiceMaster> GetB2cTargetServices()
        {
            ProcGetB2cTargetServices _proc = new ProcGetB2cTargetServices(_dal);
            List<ServiceMaster> _Services = (List<ServiceMaster>)_proc.Call();
            return _Services;
        }
                    ...
                    foreach (var ccd in data.CircleCodes)
                    {
                        if (!string.IsNullOrEmpty(ccd.CircleCode))
                        {
                            ...
                            else if (data.APICode.EndsWith(PlanType.VASFintechAPI))
                            {
                                ...
                                var _PSresp = vasPl.VASRechargePlan(ccd.CircleCode, data.APIOpCode);
                                if (_PSresp != null)
                                {
                                    if (_PSresp.StatusCode == ErrorCodes.One)
                                    {
                                        try
                                        {
                                            ...
                                            foreach (var item in _PRecords.types)
                                            {
                                                ...
                                                {
                                                    var catId = GetCategoryId(rechPT);
                                                    dtAllPlans.Rows.Add(OID, ccd.CircleID, catId, items.rs, items.validity, items.desc, DateTime.Now, DateTime.Now, APIID);
                                                }
                                            }
                                        }
                                        catch (Exception ex)
                ...
                if (data.Statuscode == ErrorCodes.One)
                {
                    ...
                    foreach (var ccd in data.CircleCodes)
                    {
                        if (!string.IsNullOrEmpty(ccd.CircleCode))
                        {
                            ...
                            if (data.APICode.EndsWith(PlanType.VASFintechAPI))
                            {
                                var vasPl = new FintechVASAPI(_accessor, _env, _dal, data.APICode);
                                var _PSresp = vasPl.VASDTHPlan(data.APIOpCode);
                                var _PRecords = JsonConvert.DeserializeObject<List<PSDthDObj>>(_PSresp.Data.ToString());
                                foreach (var item in _PRecords)
                                {
                                    var pName = item.pType;
                                    if (pName.Equals("Languages"))
                                        continue;
                                    foreach (var items in item.pDetials)
                                    {
                                        bool _IsPlan = false;
                                        string OneMonth = string.Empty, ThreeMonth = string.Empty, SixMonth = string.Empty, OneYear = string.Empty;
                                        var p = (PSDTHPrice)items.price;
                                        if (!string.IsNullOrEmpty(p.monthly))
                                        {
                                            OneMonth = p.monthly;
                                            _IsPlan = true;
                                        }
                                        if (!string.IsNullOrEmpty(p.quarterly))
                                        {
                                            ThreeMonth = p.quarterly;
                                            _IsPlan = true;
                                        }
                                        if (!string.IsNullOrEmpty(p.halfYearly))
                                        {
                                            SixMonth = p.halfYearly;
                                            _IsPlan = true;
                                        }
                                        if (!string.IsNullOrEmpty(p.yearly))
                                        {
                                            OneYear = p.yearly;
                                            _IsPlan = true;
                                        }
                                        if (!_IsPlan)
                                            continue;
                                        dtAllPlans.Rows.Add(OID, ccd.CircleID, data.APIID, items.packageName, OneMonth, ThreeMonth, SixMonth, OneYear, items.pDescription, pName, items.pLangauge, 0);
                                    }
                                }
                            }
                            if (data.APICode.Equals(PlanType.ROFFERCoPlan))
                            {
                                resp = AppWebRequest.O.CallUsingHttpWebRequest_GET(sb.ToString());
                                IProcedureAsync _err = new ProcLogPlansAPIReqResp(_dal);
                                _err.Call(new CommonReq
