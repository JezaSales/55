...
{
    public class LookupML : ILookUpPlanAPI, ILookUpGoRecharge, ILookUpRoundpay, ILookUpAPIBox, ILookUpMPLAN, ILookUpMyPlan, ILookUpVASTWEB, ILookUpInfoAPI, ILookUpAirtelPP, ILookUpROPlan
    {
        ...
        #region roffer.in
        public HLRResponseStatus GetLookUpROPlan(string URL, int APIID, int UserId, string Mobile = "")
        {
            var res = new HLRResponseStatus
            {
                Statuscode = ErrorCodes.Minus1,
            ...
            {
                APIID = APIID,
                APIType = LookupAPIType.ROFFERCoPlan,
                LoginID = UserId,
                Mobile = string.IsNullOrEmpty(MobileNo) ? Mobile : MobileNo,
                Request = URL,
                CurrentCircle = CurrentCircle,
