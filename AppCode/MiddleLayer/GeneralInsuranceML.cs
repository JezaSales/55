...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public sealed class GeneralInsuranceML
    {
        ...
        public GenerateURLResp GenerateTokenAndURLForRedirection(ValidateAPIOutletResp validateAPIOutletResp, int RequestMode)
        {
            ...
            {
                //res.RedirectURL=
                res.RedirectURL = "/GIRedirect?code=" + APICode.GIBL + "&prodid=" + Procres.RechType + "&tranid=" + (Procres.Token ?? string.Empty) + "&outlet=" + (validateAPIOutletResp.OutletID);
        ...
        public IResponseStatus DoHoldGITransaction(GIUpdateRequestModel req)
        {
            req.RequestIP = _info.GetRemoteIP();

            IProcedure proc = new ProcHoldGITransactionService(_dal);
            return (ResponseStatus)proc.Call(req);
        }
        public IResponseStatus UpdateHoldedTransaction(GIUpdateRequestModel req)
        {
            var res = new ResponseStatus();
            IProcedure proc = new ProcGetGIHoldTransactionStatus(_dal);
            var holdedResp = (GIUpdateRequestModel)proc.Call(req.TransactionID);
            if (holdedResp != null)
            {
                holdedResp.Status = req.Status == 0 ? RechargeRespType.FAILED : RechargeRespType.SUCCESS;
                holdedResp.RequestIP = _info.GetRemoteIP();
                holdedResp.Payout = req.Payout;
                holdedResp.LiveID = req.LiveID;
                proc = new ProcTransactionServiceGI_Debit(_dal);
                res= (ResponseStatus)proc.Call(holdedResp);
            }
            return res;
        }
    }
