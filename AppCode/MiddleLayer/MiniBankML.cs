...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public partial class AEPSML : IMiniBankML
    {
        ...
        public MBStatuscheckResponseApp MBStatusCheck(MBStatusCheckRequest transactionServiceReq)
        {
            ...
            {
                var RespOfDBStatus = new ResponseStatus();
                if (transactionServiceReq.TID > 0)
                {
                    IProcedure _proc = new ProcMiniBankGetDBStatus(_dal);
                    RespOfDBStatus = (ResponseStatus)_proc.Call(transactionServiceReq.TID);
                    _APICode = (RespOfDBStatus.CommonStr ?? string.Empty);
                }
                ...
                else if (_APICode.Equals(APICode.SPRINT) && RespOfDBStatus.CommonInt != RechargeRespType.SUCCESS && transactionServiceReq.RequestPage!="APPS")
                {
                    ...
                    var apiResp = aepsML.StatusCheck(new AEPSUniversalRequest
                    {
                        ...
                        IsMTM = (RespOfDBStatus.CommonStr2 ?? string.Empty) == SPKeys.MATMCashWithdrawal
                    });
                    apiReqForProc = new MiniBankTransactionServiceResp
                    {
                        LT = 1,
