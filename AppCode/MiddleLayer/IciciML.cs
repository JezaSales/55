...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class IciciML
    {
        ...
        public IResponseStatus ValidateCashfreeData(string CustomerID, string Amount, string payment_id, string status, string LiveID)
        {
            ...
            var collectML = new CashfreeCollectML(_accessor, _env, _dal, string.Empty);
            var stsCheckResp = collectML.StatusCheckICollect(payment_id);
        ...
        #region AxisBankICollect
        public ResponseStatus ValidateAxisBankRequest(AxisBankICollectReq axisBankICollectReq)
        {
            IProcedure proc = new ProcICollectValidationEntry(_dal);
            return (ResponseStatus)proc.Call(new ICollectRequest
            {
                Account = axisBankICollectReq.Sndr_acnt,
                AccountName = axisBankICollectReq.Sndr_nm,
                Amount = axisBankICollectReq.Txn_amnt,
                CorporateCode = axisBankICollectReq.Corp_code,
                IFSC = axisBankICollectReq.Sndr_Ifsc,
                IMPSAccount = axisBankICollectReq.Sndr_acnt1,
                IMPSAccountName = axisBankICollectReq.Sndr_nm1,
                PaymentMode = axisBankICollectReq.Pmode,
                UTR= axisBankICollectReq.UTR
            });
        }

        public ResponseStatus NotifyAxisBankICollect(AxisBankICollectReq axisBankICollectReq) {
            //12 Partial Succees,06 UTR rejected
            var resp = new ResponseStatus
            {
                Statuscode=ErrorCodes.Minus1
            ...
            if ((axisBankICollectReq.UTR ?? "") == "")
            {
                return resp;
            }
            if ((axisBankICollectReq.Bene_acc_no ?? "") == "")
            {
                return resp;
            }
            if (string.IsNullOrEmpty(axisBankICollectReq.Txn_amnt))
            {
                return resp;
            }
            ...
            {
                UTR = axisBankICollectReq.UTR,
                BankInternalTransactionNumber = axisBankICollectReq.UTR,
                CustomerCode = axisBankICollectReq.Corp_code,
                VirtualACCode = axisBankICollectReq.Bene_acc_no,
                CustomerAccountNo = axisBankICollectReq.Sndr_acnt,
                AMT = axisBankICollectReq.Txn_amnt,
                PayeeName = axisBankICollectReq.Sndr_nm,
                PayeeAccountNumber = axisBankICollectReq.Sndr_acnt,
                PayeePaymentDate = string.Empty,
                IsAxisBank = true,
                PayeeBankIFSC = string.Empty,
