...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class WebShoppingML : IWebShoppingML
    {
        ...
        public async Task<CheckPinCodeStatusDB> CheckDeliveryByDb(string PinCode)
        {
            var data = new CheckPinCodeStatusDB();
            try
            {
                IProcedureAsync proc = new ProcSelectPincodeForShopping(_dal);
                data = (CheckPinCodeStatusDB)await proc.Call(new CommonReq
                {
                    CommonStr = PinCode
                });
            }
