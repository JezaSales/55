...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class SellerCouponModuleML : ISellerCouponModuleML
    {
        ...
        public IResponseStatus SaveCouponVocher(CouponData data)
        {
            ...
            TransactionML tml = new TransactionML(_accessor,_env,_dal, _rinfo);
            var tResp = tml.DoTransaction(new _RechargeAPIRequest
            {
                Account= data.MobileNo,
                Amount= data.Amount,
                Optional2 = data.To,
                Optional1=data.Quantity.ToString(),
                UserID= _lr.UserID,
                CustomerNumber=data.CommonStr,
                Optional3=data.Message,
                RequestMode= RequestMode.PANEL,
                IPAddress = _rinfo.GetRemoteIP(),
                OID= data.OID
            }).Result;
            _resp.Statuscode = tResp.STATUS == RechargeRespType.FAILED ? ErrorCodes.Minus1: tResp.STATUS;
            _resp.Status = tResp.STATUS;
            _resp.Msg = tResp.MSG;
            return _resp;
        }
