        ...
        private async Task<DMTTransactionResponse> CallProcAndService(XPressServiceProcRequest PayoutRequest, DMTTransactionResponse res)
        {
            ...
            else if (forOutletAndAPICodeResp.APIOpCode.Equals(APICode.CASHFREE))
            {
                var tAPIRequest = new MTAPIRequest
                {
                    ...
                    EmailID = PayoutRequest.SenderEmail,
                    APIGroupCode = XpressSerRespProc.APIGroupCode,
                    Address = XpressSerRespProc.Address,
                    City = XpressSerRespProc.City,
                    StateName = XpressSerRespProc.StateName,
                    Pincode = Convert.ToInt32(XpressSerRespProc.Pincode)
                };
                IMoneyTransferAPIML moneyTransferAPIML = new CashFreePayoutML(_accessor, _env, _dal, forOutletAndAPICodeResp.APIGroupCode);
                APIRes = moneyTransferAPIML.AccountTransfer(tAPIRequest);
            }
            else if (forOutletAndAPICodeResp.APIOpCode.Equals(APICode.DIKSHUPay))
            {
                var tAPIRequest = new MTAPIRequest
                {
                    APIID = PayoutRequest.APIID,
                ...
                IMoneyTransferAPIML moneyTransferAPIML = new DIKSHUPayML(_accessor, _env, _dal, APICode.DIKSHUPay, forOutletAndAPICodeResp.APIID, forOutletAndAPICodeResp.APIGroupCode);
                APIRes = moneyTransferAPIML.AccountTransfer(tAPIRequest);
            }
            else if (forOutletAndAPICodeResp.APICode.Equals(APICode.INSTANTPayDirect))
            {
                var tAPIRequest = new MTAPIRequest
                {
                    APIID = PayoutRequest.APIID,
                ...
                IMoneyTransferAPIML moneyTransferAPIML = new IPayPayoutDirectML(_accessor, _env, _dal, APICode.INSTANTPayDirect, PayoutRequest.APIID, forOutletAndAPICodeResp.APIGroupCode);
                APIRes = moneyTransferAPIML.AccountTransfer(tAPIRequest);
            }
