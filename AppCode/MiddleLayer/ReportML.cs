...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public partial class ReportML : IReportML, IRechargeReportML, IDMRReportML, IRefundReportML, IAppReportML, IOutletML, IBillFetchReportML
    {
        ...
        {
            var _lst = new List<ProcRechargeReportResponse>();
            if (_lr.LoginTypeID == LoginType.ApplicationUser || userML.IsCustomerCareAuthorised(ActionCodes.ShowRechargeReport) || _lr.IsWLAPIAllowed && ApplicationSetting.IsWLAPIAllowed)
            {
                var _filter = new _RechargeReportFilter
                {
                    LT = _lr.LoginTypeID,
        ...
        public async Task<IResponseStatus> CheckStatusAsync(int TID, string TransactionID = "")
        {
            ...
            if (((_lr.RoleID == Role.Admin || (_lr.IsWLAPIAllowed && ApplicationSetting.IsWLAPIAllowed)) && _lr.LoginTypeID == LoginType.ApplicationUser) || userML.IsCustomerCareAuthorised(ActionCodes.ShowPending))
            {
                ...
                var payGRTRechReq = new PayGRTRechReq();
                if (gETStatusCheckURL.RechargeAPI.aPIDetail.APICode.Equals(APICode.PAYGRT))
                {
                    payGRTRechReq.api_ref_id = gETStatusCheckURL.TID.ToString();
                    payGRTRechReq.txn_date = Convert.ToDateTime(gETStatusCheckURL.TDate).ToString("yyyy-MM-dd");
                    payGRTRechReq.IsRecharge = false;
                }
                gETStatusCheckURL.RechargeAPI.payGRTRechReq = payGRTRechReq;
        ...
        public async Task<IResponseStatus> CheckAEPSStatusAsync(int TID, string TransactionID = "")
        {
            ...
            if ((_lr.RoleID == Role.Admin && _lr.LoginTypeID == LoginType.ApplicationUser) || userML.IsCustomerCareAuthorised(ActionCodes.ShowPending))
            {
                ...
                {
                    FingpayML fingpayML = new FingpayML(_env,_dal);
        ...
        public async Task<IResponseStatus> CheckStatusDMRAsync(int TID, string TransactionID, bool FromBulk = false)
        {
            ...
            if ((_lr.RoleID == Role.Admin && _lr.LoginTypeID == LoginType.ApplicationUser) || userML.IsCustomerCareAuthorised(ActionCodes.ShowPending))
            {
                ...
                {
                    var moneyTransferAPIML = new IPayPayoutDirectML(_accessor, _env, _dal, gETStatusCheckURL.RechargeAPI.aPIDetail.APICode, gETStatusCheckURL.RechargeAPI.aPIDetail.ID, gETStatusCheckURL.RechargeAPI.aPIDetail.GroupCode);
                    var dMRTransactionResponse = moneyTransferAPIML.GetTransactionStatus(gETStatusCheckURL.TID, gETStatusCheckURL.TransactionID, gETStatusCheckURL.RequestMode, gETStatusCheckURL.UserID, gETStatusCheckURL.RechargeAPI.aPIDetail.ID);
                    transactionStatus.Status = dMRTransactionResponse.Statuscode;
                    transactionStatus.OperatorID = dMRTransactionResponse.LiveID;
                    rechargeAPIHit.aPIDetail = new APIDetail
                ...
                else if (gETStatusCheckURL.RechargeAPI.aPIDetail.APICode == APICode.IMWALLET && (gETStatusCheckURL.RechargeAPI.aPIDetail.IsStatusBulkCheck && FromBulk || !FromBulk))
                {
                    var _apiID = gETStatusCheckURL.RechargeAPI.aPIDetail.ID;
                    var _apiGroupCode = gETStatusCheckURL.RechargeAPI.aPIDetail.GroupCode;
                    var moneyTransferAPIML = new IMWalletDMT(_accessor, _env, _dal, APICode.IMWALLET, _apiID, _apiGroupCode);
                ...
                else if (gETStatusCheckURL.RechargeAPI.aPIDetail.APICode == APICode.DIKSHUPay && (gETStatusCheckURL.RechargeAPI.aPIDetail.IsStatusBulkCheck && FromBulk || !FromBulk))
                {
                    var _apiID = gETStatusCheckURL.RechargeAPI.aPIDetail.ID;
                    var _apiGroupCode = gETStatusCheckURL.RechargeAPI.aPIDetail.GroupCode;
                    var moneyTransferAPIML = new DIKSHUPayML(_accessor, _env, _dal, APICode.DIKSHUPay, _apiID, _apiGroupCode);
        ...
        public IEnumerable<SentSmsResponse> GetSentSMSreport(SentSMSRequest req)
        {
            ...
            if (_lr.RoleID == Role.Admin && _lr.LoginTypeID == LoginType.ApplicationUser || userML.IsCustomerCareAuthorised(ActionCodes.ShowSendSMS) || _lr.IsWLAPIAllowed && ApplicationSetting.IsWLAPIAllowed)
            {
        ...
        public List<Daybook> AdminDayBook(string FromDate, string ToDate, int APIID, int OID, string Mobile_F)
        {
            ...
            if (_lr.RoleID == Role.Admin || userML.IsCustomerCareAuthorised(ActionCodes.ShowAdminDaybook) || ApplicationSetting.IsWLAPIAllowed && _lr.IsWLAPIAllowed)
            {
        ...
        public IEnumerable<APIDaybookDatewise> AdminDayBookDateAPIwise(string FromDate, string ToDate, int APIID)
        {
            ...
            if (_lr.RoleID == Role.Admin && _lr.LoginTypeID == LoginType.ApplicationUser || userML.IsCustomerCareAuthorised(ActionCodes.ShowAdminDaybook) || _lr.IsWLAPIAllowed && ApplicationSetting.IsWLAPIAllowed)
            {
                var _req = new ForDateFilter
                {
                    LT = _lr.LoginTypeID,
            ...
            if ((_lr.LoginTypeID == LoginType.ApplicationUser && (_lr.RoleID == Role.Admin || _lr.RoleID == Role.Retailor_Seller)) || userML.IsCustomerCareAuthorised(ActionCodes.ShowNumberSeries))
            {
                ...
                else if ((res.HLRAPIs != null) && (res.Statuscode == ErrorCodes.One))
                {
                    if (res.HLRAPIs.Count > 0)
                    {
                        ...
                        {
                            res = HitHLRAPIs(item.APICode, item.APIURL, item.APIID, _lr.UserID, _Number,item.APIOpCode);
        ...
        public HLRResponseStatus HitHLRAPIs(string ApiCode, string URL, int ApiId, int UserId, string MobNo,string APIOpCode)
        {
            var res = new HLRResponseStatus
            {
                Statuscode = ErrorCodes.Minus1,
            ...
            else if (ApiCode.Equals(LookupAPIType.ROFFERCoPlan))
            {
                ILookUpROPlan _ILookUp = new LookupML(_accessor, _env);
                res = _ILookUp.GetLookUpROPlan(URL, ApiId, UserId);
            }
            ...
            else if (ApiCode.EndsWith(PlanType.VASFintechAPI))
            {
                ...
                res = vasPl.VASMobileLookup(MobNo, APIOpCode, ApiId, UserId);
            }
            return res;
        }
        public HLRResponseStatus CheckNumberSeriesExist(CommonReq req)
        {
            ...
            else if ((res.HLRAPIs != null) && (res.Statuscode == ErrorCodes.One))
            {
                if (res.HLRAPIs.Count > 0)
                {
                    foreach (var item in res.HLRAPIs)
                    {
                        res = HitHLRAPIs(item.APICode, item.APIURL, item.APIID, req.LoginID, req.CommonStr,item.APIOpCode);
                        if ((res.CommonInt > 0) && !string.IsNullOrEmpty(res.CommonStr2))
                        {
                            return res;
                        }
    ...
    public partial class ReportML : IInvoiceReportML
    {
        ...
        public ResponseStatus ASPaymentCollection(ASCollectionReq aSCollectionReq)
        {
            ...
            if (aSCollectionReq.CollectionMode.ToUpper().Equals("BANK") && string.IsNullOrEmpty(aSCollectionReq.BankName))
            {
                _resp.Statuscode = ErrorCodes.Minus1;
        ...
        public async Task<PaymentGatewayEntity> GetPaymentGateway(CommonReq filter)
        {
            IProcedureAsync _proc = new ProcGetPaymentGateway(_dal);
            return (PaymentGatewayEntity)await _proc.Call(filter).ConfigureAwait(false);
        }
        ...
        public IResponseStatus ProcTogglePG(int ID)
        {
            IProcedure _proc = new ProcTogglePG(_dal);
            return (ResponseStatus)_proc.Call(new CommonReq
            {
                LoginTypeID = _lr.LoginTypeID,
                LoginID = _lr.UserID,
                CommonInt = ID
