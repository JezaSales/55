...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class CallbackML : ICallbackML, IShoppingCallback
    {
                ...
                if (!string.IsNullOrEmpty(apiRes.HookStatusKey) && (!string.IsNullOrEmpty(apiRes.HookVendorKey) || !string.IsNullOrEmpty(apiRes.HookTIDKey)) && apiRes.HookResTypeID.In(ResponseType.JSON, ResponseType.XML, ResponseType.Delimiter))
                {
                    ...
                    if (ds.Tables.Count > 0)
                    {
                        bool IsStatusFound = false,
                             IsTIDFound = string.IsNullOrEmpty(apiRes.HookTIDKey),
                             IsVendorIDFound = string.IsNullOrEmpty(apiRes.HookVendorKey),
                             IsOperatorIDFound = string.IsNullOrEmpty(apiRes.HookLiveIDKey),
                             IsMessageFound = string.IsNullOrEmpty(apiRes.HookMsgKey);
        ...
        public async Task<IResponseStatus> SaveICICIStatementAsync(PostStatetmentRequest postStatetmentRequest)
        {
            ...
            try
            {
                if (postStatetmentRequest != null)
                {
                    var statement = new List<ICICTransactionDetail>();
                    var path = DOCType.UTRStatementJsonFilePath;
                    if (File.Exists(path))
                    {
                        var jsonData = File.ReadAllText(path);
                        var config = JsonConvert.DeserializeObject<List<UtrStatementSetting>>(jsonData).Where(x => x.bankID == "33");
                        if (config == null)
                        {
                            statement = postStatetmentRequest?.data;
                        }
                        else
                        {
                            foreach (var obj in postStatetmentRequest?.data)
                            {
                                var configArray = config.ToArray();
                                for (int i = 0; i < configArray.Length; i++)
                                {
                                    if (obj.TransactionRemarks.Contains(configArray[i].identifier, StringComparison.OrdinalIgnoreCase))
                                    {
                                        var utr = string.Empty;
                                        var splits = obj.TransactionRemarks.Split(configArray[i].startWith);
                                        if (splits.Length >= 2)
                                        {
                                            utr = splits[1];
                                        }
                                        if (!string.IsNullOrEmpty(utr) && utr.Length > 0)
                                        {
                                            splits = utr.Split(configArray[i].endWith);
                                            utr = splits[0];
                                            //utr = utr.Substring(0, utr.IndexOf(configArray[i].endWith));
                                        }
                                        statement.Add(new ICICTransactionDetail
                                        {
                                            UTR = utr,
                                            AvailableBalance = obj.AvailableBalance,
                                            TransactionId = obj.TransactionId,
                                            ChequeRefNo = obj.ChequeRefNo,
                                            TransactionAmount = obj.TransactionAmount,
                                            SlNo = obj.SlNo,
                                            ValueDate = obj.ValueDate,
                                            TransactionPostedDate = obj.TransactionPostedDate,
                                            TransactionDate = obj.TransactionDate,
                                            TransactionType = obj.TransactionType,
                                            TransactionRemarks = obj.TransactionRemarks
                                        });
                                        break;
                                    }
                                }
                        ...
                        if (postStatetmentRequest.data.Count != statement.Count)
                        {
                            postStatetmentRequest.data.RemoveAll(x => x.TransactionId.In(statement.Select(y => y.TransactionId)));
                            statement = statement.Concat(postStatetmentRequest.data).ToList();
                        }
                    }
                    IProcedureAsync procedureAsync = new ProcSaveICICIStatement(_dal);
                    res = (ResponseStatus)await procedureAsync.Call(new PostStatetmentRequest
                    {
                        AccountNo = postStatetmentRequest?.AccountNo,
                        data = statement
                    }).ConfigureAwait(false);
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    ClassName = GetType().Name,
                    FuncName = "SaveICICIStatementAsync",
                    Error = ex.Message,
                    LoginTypeID = 1,
                    UserId = 1
                };
        ...
        public async Task<IResponseStatus> SaveICICIStatementNewAsync(PostStatetmentRequest postStatetmentRequest)
        {
            IResponseStatus res = new ResponseStatus
            {
                Statuscode = ErrorCodes.Minus1,
            ...
            try
            {
                if (postStatetmentRequest != null)
                {
                    var statement = new List<ICICTransactionDetail>();
                    /* ------------------ */
                    foreach(var item in postStatetmentRequest?.data)
                    {
                        /*
                         Note : OperatorId -- > UserName or UserId , TransactionID --> UTR
                         */
                        IProcedureAsync checkTextResponse = new ProcCheckTextResponse(_dal);
                        var destructedResponse = (APISTATUSCHECK)await checkTextResponse.Call(new APISTATUSCHECK { Msg = Validate.O.ReplaceAllSpecials(item.TransactionRemarks) });
                        statement.Add(new ICICTransactionDetail
                        {
                            UTR = destructedResponse.TransactionID,
                            UserName = destructedResponse.OperatorID,
                            ...
                            TransactionPostedDate = item.TransactionPostedDate,
        ...
        public async Task<string> LogCallBackRequest(CallbackData callbackData)
        {
            callbackData.RequestIP = _info.GetRemoteIP();
            callbackData.RequestBrowser = _info.GetBrowserFullInfo();
            bool response = true;
            //IProcedureAsync _procLog = new ProcSaveCallbackData(_dal);
            //var response = (bool)await _procLog.Call(callbackData).ConfigureAwait(true);
            if (response)
            {
                var _res = await UpdateViaCallback(callbackData.Content, callbackData.APIID).ConfigureAwait(false);
            }
            return "Final";
        }

        public async Task<IResponseStatus> UpdateViaCallback(string content, int APIID)
        {
            var result = new List<APIResponseTable>();
            try
            {
                Dictionary<string, object> dictionary = JsonConvert.DeserializeObject<Dictionary<string, object>>(content);
                result = ConvertResponse(dictionary, result);
                await SaveAPIResponse(new APIResponseModel { Response = result, APIID = APIID });
            }
            catch (Exception ex)
            {
            ...
            /* to be implemented
                save data into db for now.
            */
            return await Task.FromResult(new ResponseStatus { Statuscode = 1, Msg = "To be implemented" });
        }

        private List<APIResponseTable> ConvertResponse(Dictionary<string, object> dictinary, List<APIResponseTable> result, string schema = "")
        {
            try
            {
                string _schema = null;
                var list = dictinary.ToList();
                foreach (var item in list)
                {
                    if (item.Value != null)
                    {
                        var _type = item.Value.GetType().Name;
                        var typeDictionary = typeof(Dictionary<string, object>).Name;
                        var __v = new Dictionary<string, object>();
                        if (_type.Equals(typeDictionary, StringComparison.OrdinalIgnoreCase))
                            __v = (Dictionary<string, object>)item.Value;
                        if (_type == "JObject")
                            __v = ((JObject)item.Value).ToObject<Dictionary<string, object>>();
                        else if (_type == "JArray")
                        {
                            int count = 0;
                            _schema = item.Key;
                            //var oo = new List<Dictionary<string, object>>();
                            foreach (var jObj in (JArray)item.Value)
                            {
                                var o = jObj.ToObject<Dictionary<string, object>>();
                                foreach (var k in o)
                                {
                                    string __key = $"[{count}].{k.Key}";
                                    __v.Add(__key, k.Value);
                                }
                                count++;
                            }
                        }
                        if (__v != null && __v.Count > 0)
                        {
                            _type = __v.GetType().Name;
                        }
                        if (_type.Equals(typeDictionary, StringComparison.OrdinalIgnoreCase))
                        {
                            ConvertResponse(__v, result, _schema ?? item.Key);
                        }
                        else
                        {
                            result.Add(new APIResponseTable
                            {
                                Key = item.Key,
        ...
        public async Task<IResponseStatus> SaveAPIResponse(APIResponseModel apiResponseModel)
        {
            IProcedureAsync _proc = new ProcSaveAPIResponse(_dal);
            var response = (ResponseStatus)await _proc.Call(apiResponseModel).ConfigureAwait(true);
            return response;
        }
    }
}
