...
using RoundpayFinTech.AppCode.ThirdParty.FintechVASAPI;
using System;
...
namespace RoundpayFinTech.AppCode.MiddleLayer
{
    public class PlansAPIML : IMplan, IRoundpayPlan, IPlanAPIPlan, ICyrusAPIPlan, IVastWebPlan, IMyPlanAPI
    {
        ...
        #region RNPPlans
        public List<RNPRechPlansPanel> GetRNPRechargePlans(int oid, int circleId,int LT,int UserID)
        {
            ...
            if (IsLivePlan)
            {
                rJData = GetLiveRechargePlans(oid, circleId, LT, UserID);
            }
            else
            {
        ...
        private List<RNPRechPlansPanel> GetLiveRechargePlans(int oid, int circleId,int LT,int UserID)
        {
            ...
            if (data.Statuscode == ErrorCodes.One)
            {
                ...
                else if (data.APICode.EndsWith(PlanType.VASFintechAPI))
                {
                    var vasPl = new FintechVASAPI(_accessor, _env, _dal, data.APICode);
                    var _PSresp = vasPl.VASRechargePlan(cir[0].CircleCode, data.APIOpCode);
                    if (_PSresp != null)
                    {
                        if (_PSresp.StatusCode == ErrorCodes.One)
                        {
                            try
                            {
                                var _PRecords = JsonConvert.DeserializeObject<PSRechObj>(_PSresp.Data.ToString());
                                foreach (var item in _PRecords.types)
                                {
                                    var rechPT = item.pType;
                                    foreach (var items in item.pDetails)
                                    {
                                        rLivePlans.Add(new RNPRechPlansPanel
                                        {
                                            OID = oid,
                                            CircleID = circleId,
                                            RechargePlanType = rechPT,
                                            RAmount = items.rs,
                                            Details = items.desc,
                ...
                else if (data.APICode.Equals(PlanType.ROFFERCoPlan))
                {
                    resp = AppWebRequest.O.CallUsingHttpWebRequest_GET(sb.ToString());
                    IProcedureAsync _err = new ProcLogPlansAPIReqResp(_dal);
                    _err.Call(new CommonReq
        ...
        public RNPDTHPlans GetRNPDTHPlans(int oid, int LT, int UserID)
        {
            var dthPlansresp = new RNPDTHPlans();
            IOperatorML op = new OperatorML(_accessor, _env);
            dthPlansresp = GetRNPDTHPlansPVT(oid, LT, UserID);
            var cDate = DateTime.Now.ToString("dd MMM yyyy");
        ...
        private RNPDTHPlans GetRNPDTHPlansPVT(int o,int LT,int UserID)
        {
        ...
        public RNPRoffer GetRNPRoffer(string m, int o)
        {
            ...
            if (resp.StatusCode == ErrorCodes.One)
            {
                try
                {
                    ...
                    {
                        resp = HitRofferAPIs(item.APICode, item.APIURL, m, item.APIOpCode);
                        if (resp.StatusCode == ErrorCodes.One)
                        {
                            break;
                        }
        ...
        private RNPRoffer HitRofferAPIs(string APICode, string URL, string Mob, string APIOpCode)
        {
            var resp = new RNPRoffer
            {
                StatusCode = ErrorCodes.Minus1,
            ...
            if (!APICode.EndsWith(PlanType.VASFintechAPI))
            {
                ...
                webResult = AppWebRequest.O.CallUsingHttpWebRequest_GET(URL);
            ...
            else if (APICode.EndsWith(PlanType.VASFintechAPI))
            {
                var vasPl = new FintechVASAPI(_accessor, _env, _dal, APICode);
                resp = vasPl.VASBestOffer(Mob, APIOpCode);
            }
            else if (APICode.Equals(PlanType.ROFFERCoPlan))
            {
                if (webResult != "")
                {
                    if (webResult.Contains("You are not authorize"))
        ...
        public RNPDTHCustInfo GetRNPDTHCustInfo(int o, string a)
        {
            ...
            if (resp.StatusCode == ErrorCodes.One)
            {
                try
                {
                    foreach (var item in data.HLRAPIs)
                    {
                        resp = HitDthCustInfoAPI(item.APICode, item.APIURL, a, item.APIOpCode);
                        if (resp.StatusCode == ErrorCodes.One)
                        {
                            break;
                        }
        ...
        private RNPDTHCustInfo HitDthCustInfoAPI(string APICode, string URL, string Acc, string APIOpCode)
        {
            var resp = new RNPDTHCustInfo
            {
                StatusCode = ErrorCodes.Minus1,
            ...
            else if (APICode.Equals(PlanType.PLANAPI))
            {
                if (!string.IsNullOrEmpty(webResult))
                {
                    ...
                    {
                        var resJ = DynamicHelper.O.GetKeyValuePairs(webResult, "DATA", false);
                        resp.Data = JsonConvert.DeserializeObject<List<RNPDTHCustList>>(JsonConvert.SerializeObject(resJ));
                        resp.StatusCode = ErrorCodes.One;
                        resp.Msg = ErrorCodes.SUCCESS;
                    }
            ...
            {
                var vasPl = new FintechVASAPI(_accessor, _env, _dal, APICode);
                resp = vasPl.VASDTHCustomerInfo(Acc, APIOpCode);
            }
            else if (APICode.Equals(PlanType.ROFFERCoPlan))
            {
                if (!string.IsNullOrEmpty(webResult))
        ...
        public RNPDTHHeavyRefresh GetRNPDTHHeavyRefresh(int o, string a)
        {
            ...
            if (resp.StatusCode == ErrorCodes.One)
            {
                try
                {
                    foreach (var item in data.HLRAPIs)
                    {
                        resp = HitDTHHeavyRefreshAPI(item.APICode, item.APIURL, a, item.APIOpCode);
                        if (resp.StatusCode == ErrorCodes.One)
                        {
                            break;
                        }
        ...
        private RNPDTHHeavyRefresh HitDTHHeavyRefreshAPI(string APICode, string URL, string Acc, string APIOpCode)
        {
            var resp = new RNPDTHHeavyRefresh
            {
                StatusCode = ErrorCodes.Minus1,
            ...
            {
                var vasPl = new FintechVASAPI(_accessor, _env, _dal, APICode);
                resp = vasPl.VASDTHHeavyRefresh(Acc, APIOpCode);
            }
            return resp;
        }
        public PSHLRResponse CheckNumberSeriesExist(string _Number, string _SpKey, int _UserId)
        {
            var res = new PSHLRResponse
            {
                StatusCode = ErrorCodes.Minus1,
            ...
            var _res = (HLRResponseStatus)_proc.Call(req);
            if ((_res.CommonInt > 0 && _res.CommonInt2 > 0) && string.IsNullOrEmpty(_res.CommonStr3))
            {
                return res;
            }
            else if ((_res.HLRAPIs != null) && (_res.Statuscode == ErrorCodes.One))
            {
                if (_res.HLRAPIs.Count > 0)
                {
                    foreach (var item in _res.HLRAPIs)
                    {
                        res = HitHLRAPIs(item.APICode, item.APIURL, item.APIID, _UserId, _Number);
                        if (res.StatusCode == ErrorCodes.One)
                        {
                            break;
                        }
        ...
        public PSHLRResponse HitHLRAPIs(string ApiCode, string URL, int ApiId, int UserId, string MobNo)
        {
            var res = new PSHLRResponse
            {
                StatusCode = ErrorCodes.Minus1,
            ...
            _err.Call(new CommonReq
            {
                CommonStr = "PlanServices-MobileLookup",
                CommonStr2 = URL,
                CommonStr3 = webResult
            });
            if (string.IsNullOrEmpty(webResult))
            ...
            try
            {
                if (ApiCode.Equals(LookupAPIType.PLANAPI))
                {
                    ...
                    var lookUpRes = JsonConvert.DeserializeObject<LookUpRes>(webResult);
                    res.Circle = lookUpRes.Circle;
                    res.Operator = lookUpRes.Operator;
                    res.lookup_number = MobNo;
                ...
                else if (ApiCode.Equals(LookupAPIType.GoRecharge))
                {
                    ...
                    var t = _ILookUp.GetLookUpGoRecharge(URL, ApiId, UserId, MobNo);
                }
                else if (ApiCode.Equals(LookupAPIType.Roundpay))
                {
                    ILookUpRoundpay _ILookUp = new LookupML(_accessor, _env);
                    var lookUpRes = JsonConvert.DeserializeObject<LookUpRoundpayRes>(webResult);
                    if (lookUpRes != null && lookUpRes.response != null)
                    {
                        if (lookUpRes.response[0].ported.ToString().ToUpper() == "TRUE")
                        {
                            res.lookup_number = MobNo;
                            res.Circle = lookUpRes.response[0].new_circle;
                            res.Operator = lookUpRes.response[0].new_operator;
                        }
                        else
                        {
                            res.lookup_number = MobNo;
                            res.Circle = lookUpRes.response[0].old_circle;
                            res.Operator = lookUpRes.response[0].old_operator;
                        }
                        res.StatusCode = ErrorCodes.One;
                        res.Msg = ErrorCodes.SUCCESS;
                    }
                ...
                else if (ApiCode.Equals(LookupAPIType.APIBox))
                {
                    ...
                    var lookUpRes = JsonConvert.DeserializeObject<LookUpAPIBoxRes>(webResult);
                    if (lookUpRes != null && lookUpRes.response != null)
                    {
                        if (!lookUpRes.response.status_code.Equals("SPE"))
                        {
                            if (lookUpRes.response.ported ?? false)
                            {
                                ...
                                {
                                    res.lookup_number = MobNo;
                                    res.Circle = lookUpRes.response.new_operator_circle.Split(',')[1];
                                    if (lookUpRes.response.status_code.In("SDL", "SAC"))
                                    {
                                        res.Operator = lookUpRes.response.new_operator_circle.Split(',')[0];
                            ...
                            else
                            {
                                ...
                                {
                                    res.lookup_number = MobNo;
                                    res.Circle = lookUpRes.response.operator_circle.Split(',')[1];
                                    if (lookUpRes.response.status_code.In("SDL", "SAC"))
                                    {
                                        res.Operator = lookUpRes.response.operator_circle.Split(',')[0];
                                        //_IsCircleOnly = lookUpRes.response.status_code.Equals("SAC") ? true : false;
                                    }
                                }
                            }
                ...
                else if (ApiCode.Equals(LookupAPIType.MPLAN))
                {
                    ...
                    var lookUpRes = JsonConvert.DeserializeObject<LookupMPlanAPIResponse>(webResult);
                    if (lookUpRes != null && lookUpRes.records != null)
                    {
                        if (lookUpRes.records.status == 1)
                        {
                            res.lookup_number = MobNo;
                            //res.Circle = lookUpRes.records.circle;
                            res.Circle = lookUpRes.records.comcircle;
                            res.Operator = lookUpRes.records.Operator;
                            res.StatusCode = ErrorCodes.One;
                            res.Msg = ErrorCodes.SUCCESS;
                        }
                    }
                ...
                else if (ApiCode.Equals(LookupAPIType.MYPLAN))
                {
                    ...
                    var lookUpRes = JsonConvert.DeserializeObject<HlrLookUPMyPlan>(webResult);
                    if (lookUpRes != null && lookUpRes.result.records != null)
                    {
                        if (lookUpRes.result.records.status == 1)
                        {
                            res.lookup_number = MobNo;
                            //res.Circle = lookUpRes.records.circle;
                            res.Circle = lookUpRes.result.records.comcircle;
                            res.Operator = lookUpRes.result.records.@operator;
                            res.StatusCode = ErrorCodes.One;
                            res.Msg = ErrorCodes.SUCCESS;
                        }
                    }
                ...
                else if (ApiCode.Equals(LookupAPIType.VASTWEB))
                {
                    ...
                    var lookUpRes = JsonConvert.DeserializeObject<VastWebHLRResp>(webResult);
                    if (lookUpRes != null && lookUpRes.Response != null)
                    {
                        if (lookUpRes.status.ToString().ToUpper().Equals("SUCCESS"))
                        {
                            res.lookup_number = MobNo;
                            res.Circle = lookUpRes.Response.Circle;
                            res.Operator = lookUpRes.Response.Operator;
                            res.StatusCode = ErrorCodes.One;
                            res.Msg = ErrorCodes.SUCCESS;
                        }
                    }
                ...
                else if (ApiCode.Equals(LookupAPIType.INFOAPIHLR))
                {
                    ...
                    var lookUpRes = JsonConvert.DeserializeObject<InfoAPIHLRResp>(webResult);
                    if (lookUpRes != null && lookUpRes.data != null)
                    {
                        if (lookUpRes.status.ToString().ToUpper().Equals("SUCCESS"))
                        {
                            res.lookup_number = MobNo;
                            res.Circle = lookUpRes.data.state.ToString();
                            res.Operator = lookUpRes.data.@operator.ToString();
                            res.StatusCode = ErrorCodes.One;
                            res.Msg = ErrorCodes.SUCCESS;
                        }
                    }
            ...
            {
                webResult = webResult + ex.Message;
                var _ = new ProcPageErrorLog(_dal).Call(new ErrorLog
                {
                    ClassName = GetType().Name,
                    FuncName = "PlanServices - HitHLRAPIs",
                    Error = webResult,
                    LoginTypeID = 1,
                    UserId = 1
                });
            ...
            var __ = (HLRResponseStatus)proc.Call(new LookUpDBLogReq
            {
                APIID = ApiId,
        ...
        #region AppPlans
        public object AppSimplePlan(int oid, int circleId,int LT,int UserId)
        {
            var planData = new List<RNPRechPlansPanel>();
            var getAS = new SettingML(_accessor, _env);
            bool IsLivePlan = getAS.GetASSettings().IsRechargePlansAPIOnly;
            if (IsLivePlan)
            {
                planData = GetLiveRechargePlans(oid, circleId, LT, UserId);
            }
            else
            { 
                planData = GetRNPRechargePlansPVT(oid, circleId);
            }

            #region PlanImplementation
        ...
        {
            IProcedure proc = new ProcGetCircleForPlanServices(_dal);
            return (List<PSCircleCode>)proc.Call();
        }
        public List<PSOperatorCode> PSGetOperatorCode()
        {
            IProcedure proc = new ProcGetOPIDForPlanServices(_dal);
            return (List<PSOperatorCode>)proc.Call();
        }
        #endregion
        ...
        #region roffer.in
        public SubMplanRofferResp GetRRoffer(string AccountNo, int OID)
        {
            var subMplanRofferResp = new SubMplanRofferResp();
            if ((AccountNo ?? "") != "" && OID > 0)
            {
        ...
        public SubMplanDTHCustomerInfo GetRDTHCustInfo(string AccountNo, int OID)
        {
            var respDTHPInfo = new SubMplanDTHCustomerInfo();
            var req = new CommonReq
            {
        ...
        public SubMplanSimplePlanResp GetRoSimplePlan(int CircleID, int OID)
        {
            var subMplanSimplePlanResp = new SubMplanSimplePlanResp();
            if (CircleID > 0 && OID > 0)
            {
        ...
        public SubMplanDTHSimplePlanResp GetRoDTHSimplePlan(int OID)
        {
            var subMplanDTHSimplePlanResp = new SubMplanDTHSimplePlanResp();
            var req = new CommonReq
            {
        ...
        public MplanDTHHeavyRefresh GetRoDTHHeavyRefresh(string AccountNo, int OID)
        {
            var respDTHHeavyRefresh = new MplanDTHHeavyRefresh();
            var req = new CommonReq
            {
