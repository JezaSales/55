    ...
    public class TransactionHelper
    {
        ...
        public async Task<TransactionStatus> HitGetStatus(int TID, TransactionServiceReq TransRequest, APIDetail currentAPI, List<APIDetail> moreAPIs, bool IsReplaceCurrent, TransactionServiceResp TransResponse)
        {
            ...
            foreach (var API in moreAPIs)
            {
                ...
                if (_HitAPI.aPIDetail.APICode == APICode.PAYGRT)
                {
                    _HitAPI.payGRTRechReq = new PayGRTRechReq
                    {
                        recharge_number = TransRequest.AccountNo,
                        operator_code = _HitAPI.aPIDetail.APIOpCode,
                        recharge_amount = TransRequest.AmountR.ToString(),
                        api_ref_id = TID.ToString(),
                        IsRecharge = true
                    };
                }
        ...
        public async Task<RechargeAPIHit> HitRechargeAPI(RechargeAPIHit rechargeAPI, TransactionServiceReq TransRequest, TransactionServiceResp TransResponse)
        {
            ...
            try
            {
                ...
                else if (rechargeAPI.aPIDetail.APICode == APICode.PAYGRT)
                {
                    PayGRT_ML objPayGRT_ML = new PayGRT_ML(APICode.PAYGRT,_env);
                    if(rechargeAPI.payGRTRechReq.IsRecharge)
                        rechargeAPIHit.Response = await objPayGRT_ML.PayGRTRecharge(this, rechargeAPI).ConfigureAwait(false);
                    else
                        rechargeAPIHit.Response = await objPayGRT_ML.PayGRTStatusCheck(this, rechargeAPI).ConfigureAwait(false);
        ...
        public async Task ResponseHandlerFetchBill(BBPSResponse bBPSResponse, BBPSLog bBPSLog)
        {
            ...
            try
            {
                ...
                if (ds.Tables.Count > 0)
                {
                    ...
                    foreach (DataTable tbl in ds.Tables)
                    {
                        ...
                        if (!IsBillAmountKeyFound && BAK.CommonBool)
                        {
                            ...
                            IsStatusFound = true;
                            bBPSLog.Amount = Convert.ToDecimal(string.IsNullOrEmpty(bBPSResponse.Amount) ? "0" : bBPSResponse.Amount);
                        }
                ...
                bBPSResponse.Amount = string.IsNullOrEmpty(bBPSResponse.Amount) ? "0" : bBPSResponse.Amount;
                if (Validate.O.IsNumeric(bBPSResponse.Amount.Replace(".", "")))
                {
                ...
                bBPSResponse.ErrorMsg = (bBPSResponse.ErrorMsg ?? string.Empty).Replace(Replacement.REPLACE, APIMsg);
        ...
        public async Task<APIBalanceResponse> HitGetBalance(APIDetail aPIDetail)
        {
            ...
            try
            {
                if ((aPIDetail.BalanceURL ?? string.Empty).Length > 0)
                {
                    ...
                    else if (aPIDetail.APICode.Equals(APICode.FINO))
                    {
                        var finML = new FINOML(_accessor, _env, _dal);
                        balanceResponse = finML.GetBalance();
                    }
                    else
                    {
