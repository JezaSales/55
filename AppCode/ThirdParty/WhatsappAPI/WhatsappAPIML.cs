...
namespace RoundpayFinTech.AppCode.ThirdParty.WhatsappAPI
{
    public class WhatsappAPIML
    {
        ...
        private WhatsappSetting AppSetting()
        {
            ...
            try
            {
                setting.Wati = new Wati
                {
                    BaseUrl = Configuration["WHATSAPP:WATI:BaseUrl"],
                    BearerToken = Configuration["WHATSAPP:WATI:BearerToken"]
                };
                setting.Wateam = new Wateam
                {
                    Url = Configuration["WHATSAPP:WATEAM:URL"],
                    APIKEY = Configuration["WHATSAPP:WATEAM:API-KEY"],
                    WACID = Configuration["WHATSAPP:WATEAM:WA-CID"]
                };
                setting.ThreeSixty = new ThreeSixty
                {
                    APIKEY = Configuration["WHATSAPP:THREESIXTY:API-KEY"],
                    Url = Configuration["WHATSAPP:THREESIXTY:URL"]
                };
            }
            catch (Exception ex)
            {
        ...
        public async Task<SendSessionMessageResponse> SendSessionMessage(string messageText, string WhatsappNumber)
        {
            ...
            try
            {
                string _URL = appSetting.Wati.BaseUrl + "sendSessionMessage/" + WhatsappNumber + "?" + "messageText=" + messageText;
                var header = new Dictionary<string, string>
                {
                    { "Authorization","Bearer "+appSetting.Wati.BearerToken },
        ...
        public async Task<SendSessionMessageResponse> WATEAM_SendSessionMessage(WhatsappAPIWaTeam _ObjWteam)
        {
            ...
            try
            {
                string _URL = appSetting.Wateam.Url;
                var header = new Dictionary<string, string>
                {
                    { "API-KEY",appSetting.Wateam.APIKEY},
                    { "WA-CID",appSetting.Wateam.WACID}
                };
                StringBuilder respex = new StringBuilder("");
                var resp = "";
        ...
        #endregion
        #region ThreeSixty
        public async Task<SendSessionMessageResponse> ThreeSixtySendSessionMessage(WhatsappAPIWaTeam _ObjWteam)
        {
            var _Request = string.Empty;
            var _Response = string.Empty;
            var reserr = new SendSessionMessageResponse
            {
                ...
                result = ErrorCodes.FAILED,
                info = ErrorCodes.FAILED
            };
            try
            {
                string _URL = appSetting.ThreeSixty.Url;
                var header = new Dictionary<string, string>
                {
                    { "D360-API-KEY",appSetting.ThreeSixty.APIKEY}
                };
                StringBuilder respex = new StringBuilder("");
                var resp = "";
                resp = await AppWebRequest.O.PostJsonDataUsingHWRTLS(_URL, _ObjWteam, header).ConfigureAwait(false);
                ...
                if (!string.IsNullOrEmpty(resp))
                {
                    if (resp.Contains("developer_message"))
                    {
                        var failres = JsonConvert.DeserializeObject<WhatsappThreeSixtyRespFailed>(resp);
                        if (failres.meta.success == false)
                        {
                            reserr.statuscode = ErrorCodes.Minus1;
                            reserr.result = ErrorCodes.FAILED;
                            reserr.info = ErrorCodes.FAILED;
                        }
                    }
                    else
                    {
                        var failres = JsonConvert.DeserializeObject<WhatsappThreeSixtyResp>(resp);
                        reserr.statuscode = 1;
                        reserr.result = "success";
                        reserr.info = "success";
                        foreach (Message item in failres.messages)
                        {
                            reserr.conversationId = item.id;
                        }
                    }
        ...
        public async Task<string> GetToken()
        {
            ...
            {
                string token = "Bearer ";
                token = token + appSetting.Wati.BearerToken;
                return token;
            }
            catch (Exception ex)
            {
        ...
        public async Task<SendSessionMessageResponse> sendSessionFileMessage(string messageText, string WhatsappNumber, IFormFile fd, string mimetype, string filename)
        {
            ...
            try
            {
                ...
                _URL = appSetting.Wati.BaseUrl + "sendSessionFile/" + WhatsappNumber + "?" + "caption=" + messageText;
                if (mimetype == "image")
                {
                    mimetype = "image/" + "png";
                }
                ...
                if (!string.IsNullOrEmpty(resp))
                {
                    ...
                    else
                    {
                        ...
                        SendSessionMessageResponse.message.text = appSetting.Wati.BaseUrl + "getMedia?fileName=" + SendSessionMessageResponse.message.text;
                        return SendSessionMessageResponse;
                    } // SendSessionMessageResponse.message = "";
        ...
        public async Task<WhatsappGetContactResp> GetContactList()
        {
            ...
            try
            {
                string _URL = appSetting.Wati.BaseUrl + "getContacts";
                var header = new Dictionary<string, string>
                {
                    { "Authorization","Bearer "+appSetting.Wati.BearerToken },
                    { "Content-Type","multipart/form-data"},
        ...
        public async Task<WhatsappGetMessageTempResp> WhatsappGetMessageTemplate()
        {
            ...
            try
            {
                string _URL = appSetting.Wati.BaseUrl + "getMessageTemplates";
                var header = new Dictionary<string, string>
                {
                    { "Authorization","Bearer "+appSetting.Wati.BearerToken },
                    { "Content-Type","multipart/form-data"},
