        ...
        public GIUpdateRequestModel SerializeDebitRequest(string reqJson)
        {
            ...
            if (reqJson.Contains("ptype"))
            {
                ...
                if (!string.IsNullOrEmpty(apiResp.ptype))
                {
                    ...
                    resp.RechType = ((apiResp.ptype ?? String.Empty) == "TWT" ? OPTypes.TwoWheeler : OPTypes.FourWheeler).ToString();
                    ...
                    resp.OutletID = Validate.O.IsNumeric(apiResp.urc) ? Convert.ToInt32(apiResp.urc) : Convert.ToInt32(apiResp.urc.Replace("R", ""));
