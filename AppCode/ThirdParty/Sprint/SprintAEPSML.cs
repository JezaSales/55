...
namespace RoundpayFinTech
{
    public partial class SprintBBPSML
    {
                    ...
                    if (apiresp.status)
                    {
                        if (apiresp.response_code == 1)
                        {
                            ...
                            if (apiresp.ministatement != null)
                            {
                                ...
                                {
                                    var l = new List<MiniStatement>();
                                    foreach (var item in apiresp.ministatement)
                                    {
                                        l.Add(new MiniStatement
                                        {
        ...
        public MiniBankTransactionServiceResp StatusCheck(AEPSUniversalRequest modelRequest)
        {
            ...
            var _URL = appSetting.AEPSBaseURL + (modelRequest.IsMTM ? "matm/matmquery/query" : "aeps/aepsquery/query");
            var req = new
            {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(response))
                {
                    ...
                    res.LiveID = apiresp.bankrrn;
                    res.CardNumber = string.IsNullOrEmpty(apiresp.cardnumber)? "XXXXXXXXXXXX": apiresp.cardnumber;
                    res.BankTransactionDate = string.Empty;
                    res.BankName =string.IsNullOrEmpty(apiresp.bankiin)? apiresp.bankName: apiresp.bankiin;
                    if (apiresp.status == true)
                    {
        ...
        public MiniBankTransactionServiceResp StatusCheckThreeWay(AEPSUniversalRequest modelRequest)
        {
            ...
            var _URL = appSetting.AEPSBaseURL + (modelRequest.IsMTM ? "matm/threeway/update" : "aeps/threeway/threeway");
