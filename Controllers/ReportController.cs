...
namespace RoundpayFinTech.Controllers
{
    ...
    public partial class ReportController : Controller
    {
        ...
        [HttpPost]
        [Route("BBPS-ServiceRoute")]
        public IActionResult BBPSServiceRoute()
        {
            ISlabML slabML = new SlabML(_accessor, _env);
            var res = slabML.BBPSServiceRoute();
            return PartialView("PartialView/_BBPSServiceRoute", res);
        }
        [HttpPost]
        [Route("BBPS-ServiceRoute-Save")]
        public IActionResult BBPSServiceRouteSave(int sr, int srp)
        {
            ...
            ISlabML slabML = new SlabML(_accessor, _env);
            _resp = slabML.BBPSServiceRouteSave(sr, srp);
            return Json(_resp);
        }
        [HttpPost]
        [Route("rslab-detail")]
        ...
        {
            if (ApplicationSetting.IsECollectEnable || ApplicationSetting.IsUPIQR || ApplicationSetting.IsUPI)
            {
                var smartCollectML = new SmartCollectML(_accessor, _env);
                var _ = smartCollectML.GetSmartVirtualAccountDetail(_lr.UserID);
            }
        ...
        public IActionResult MyBalance()
        {
            ...
            sb.Append("[");
            sb.Append(ub.PrepaidWalletName);
            sb.AppendFormat(" : {0}", ub.Balance);
            if (ub.IsBBalance && _lr.RoleID != Role.Admin)
            {
                sb.Append(" | ");
                sb.Append(ub.BankWalletName);
                sb.AppendFormat(" : {0}", ub.BBalance);
            }
            if (ub.IsUBalance)
            {
                sb.Append(" | ");
                sb.Append(ub.UtilityWalletName);
                sb.AppendFormat(" : {0}", ub.UBalance);
            }
            if (ub.IsPacakgeBalance)
            {
                sb.Append(" | ");
                sb.Append(ub.PackageWalletName);
                sb.AppendFormat(" : {0}", ub.PacakgeBalance);
            }
            if (ub.IsCBalance)
            {
                sb.Append(" | ");
                sb.Append(ub.CardWalletName);
                sb.AppendFormat(" : {0}", ub.CBalance);
            }
            if (ub.IsIDBalance)
            {
                sb.Append(" | ");
                sb.Append(ub.RegIDWalletName);
                sb.AppendFormat(" : {0}", ub.IDBalnace);
            }
        ...
        {
            if ((_lr.LoginTypeID == LoginType.ApplicationUser && _lr.RoleID == Role.Admin || userML.IsCustomerCareAuthorised(ActionCodes.ShowRefundRequestHistory)) || _lr.IsWLAPIAllowed && ApplicationSetting.IsWLAPIAllowed)
            {
                return View();
            }
            else
        ...
        public IActionResult PGRedirect(int id, int a, int w, int pg)
        {
            if (_lr.RoleID != Role.Admin && _lr.LoginTypeID == LoginType.ApplicationUser && ApplicationSetting.IsAddMoneyEnable)
            {
                ...
                IPaymentGatewayML gatewayML = new PaymentGatewayML(_accessor, _env);
                var res = gatewayML.IntiatePGTransactionForWeb(_lr.UserID, a, pg, id, w, _WInfo.WID == 1 ? _WInfo.AbsoluteHost : _WInfo.MainDomain, string.Empty);
                return View(res);
            }
        ...
        {
            int wid = _lr != null ? _lr.WID : _lrEmp.WID;
            IUserWebsite _userML = new UserML(_accessor, _env);
            var resp = _userML.GetCompanyProfile(wid);
        ...
        {
            IBannerML resourceML = new ResourceML(_accessor, _env);
            IResponseStatus responseStatus = resourceML.uploadTinyMCEImage(file, _lr.WID);
            return Json(responseStatus.CommonStr);
        }
    }
}
