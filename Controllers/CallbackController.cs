...
namespace RoundpayFinTech.Controllers
{
    ...
    public partial class CallbackController : Controller
    {
        ...
        [Route("Callback/GI/GIBLPremiumDeduction")]
        [Route("GI/GIBLPremiumDeduction")]
        public IActionResult GIBLPremiumDeduction()
        {
            ...
            if (Is)
            {
                ...
                if (serializedRes.Status > 0)
                {
                    var GIML = new GeneralInsuranceML(_accessor, _env);
                    var giRes = GIML.DoHoldGITransaction(serializedRes);
                    gIBLPremDeductResponse.status = giRes.Statuscode == ErrorCodes.One ? 1 : 0;
                    gIBLPremDeductResponse.message = gIBLPremDeductResponse.status == ErrorCodes.One ? RechargeRespType._SUCCESS : giRes.Msg;
                    gIBLPremDeductResponse.refno = giRes.CommonInt.ToString();
                    gIBLPremDeductResponse.resptime = DateTime.Now.ToString("dd MMM yyyy hh:mm:ss");
                }
            }
            SaveReqResp(ServiceType.GenralInsurance, JsonConvert.SerializeObject(gIBLPremDeductResponse));
            return Json(gIBLPremDeductResponse);
        }

        [Route("Callback/GI/GIBLConfirmation")]
        [Route("GI/GIBLConfirmation")]
        public IActionResult GIBLConfirmation()
        {
            ...
            if (Is)
            {
                ...
                if ((serializedRes.Status??-1) > -1)
                {
                    var GIML = new GeneralInsuranceML(_accessor, _env);
                    var giRes = GIML.UpdateHoldedTransaction(serializedRes);
                    gIBLPremDeductResponse.status = giRes.Statuscode == ErrorCodes.One ? 1 : 0;
                    gIBLPremDeductResponse.message = gIBLPremDeductResponse.status == ErrorCodes.One ? RechargeRespType._SUCCESS : giRes.Msg;
                }
            }
        ...
        [Route("Callback/ThreeSixty/{wpsender}")]
        [Route("ThreeSixty/{wpsender}")]
        public IActionResult ThreeSixty(int wpsender)
        {
            StringBuilder resp = new StringBuilder("");
            var request = HttpContext.Request;
            var callbackAPIReq = new CallbackData
        ...
        [Route("callback/AfCallback")]
        public IActionResult AfCallback()
        {
            StringBuilder resp = new StringBuilder("");
            var callbackRequest = GetDataFromRequest(HttpContext.Request);
            ICallbackML ml = new CallbackML(_accessor, _env);
            ml.LogCallBackRequest(callbackRequest).ConfigureAwait(false);
            return Ok();
        }

        private CallbackData GetDataFromRequest(HttpRequest httpRequest)
        {
            StringBuilder resp = new StringBuilder("");
            try
            {
                if (httpRequest.Method == "POST")
                {
                    if (httpRequest.HasFormContentType)
                    {
                        if (httpRequest.Form.Keys.Count > 0)
                        {
                            ...
                            {
                                httpRequest.Form.TryGetValue(item, out StringValues strVal);
                                if (resp.Length == 0)
                                {
                                    resp.AppendFormat("{0}={1}", item, strVal);
                                }
                    ...
                    {
                        resp = new StringBuilder(httpRequest.GetRawBodyStringAsync().Result);
                    }
                }
                else
                {
                    resp = new StringBuilder(httpRequest.QueryString.ToString());
                }
                if (resp.Length == 0)
                {
                    resp = new StringBuilder(httpRequest.QueryString.ToString());
            ...
            return new CallbackData
            {
                ...
                Content = WebUtility.UrlDecode(resp.ToString()),
                Scheme = httpRequest.Scheme,
                Path = httpRequest.Path
            };
        }

        [Route("callback/Affiliate/{id}")]
        public IActionResult Affiliate(int id)
        {
            var callbackRequest = GetDataFromRequest(HttpContext.Request);
            if(HttpContext.Request.Method.ToUpper() == "GET")
            {
                var dict = System.Web.HttpUtility.ParseQueryString(callbackRequest.Content);
                var json = JsonConvert.SerializeObject(dict.AllKeys.ToDictionary(k => k, k => dict[k]));
                callbackRequest.Content = json;
            }
            callbackRequest.APIID = id;
            ICallbackML ml = new CallbackML(_accessor, _env);
            ml.LogCallBackRequest(callbackRequest).ConfigureAwait(false);
            return Ok("done");
        }
