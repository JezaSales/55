    ...
    public class WebAppController : Controller
    {
        ...
        private HeaderInfo ReadHeader()
        {
            ...
            try
            {
                ...
                HttpContext.Request.Headers.TryGetValue(nameof(headerInfo.UserID), out var IsTotal);
        ...
        public IActionResult GetMembershipType()
        {
            ...
            appResp.IsPasswordExpired = appResponse.IsPasswordExpired;
            appResp.Statuscode = appResponse.Statuscode;
            appResp.IsB2cMembershipRePurchase = ApplicationSetting.IsB2cMembershipRePurchase;
            if (appResp.Statuscode == ErrorCodes.One)
            {
                IMembershipML memberML = new MembershipML(_accessor, _env, false);
                var getB2CRes = memberML.GetB2CMemberShipType(readHeader.UserID);
                appResp.MemberTypes = getB2CRes.membershipmasterB2Cs;
                //appResp.DetailsServiceWise = getB2CRes.slabdetailServicewises;
            }
        ...
        public async Task<IActionResult> WalletToWalletTransfer([FromBody] PGWebRequestModel requestModel)//for wallet
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                ...
                {
                    IUserML userML = new UserML(_accessor, _env, false);
                    if (ApplicationSetting.Isb2cWalletToWalletOtp)
                    {
                        if (requestModel.otprefrenceid < 1)
                        {
                            requestModel.otp = HashEncryption.O.CreatePasswordNumeric(6);
                        }
                        var Otpreq = new CommonReq
                        {
                            CommonInt = requestModel.otprefrenceid,
                            CommonStr = requestModel.otp,
                            CommonInt3 = paymentmode.UserID
                        };
                        var alertData = userML.GetUserDeatilForAlert(paymentmode.UserID);
                        var OtpRes = userML.Generateb2cOtp(Otpreq);
                        if (OtpRes.Statuscode != ErrorCodes.One)
                        {
                            if (OtpRes.Statuscode == ErrorCodes.Two)
                            {
                                //Send Sms And Email
                                IAlertML alertMl = new AlertML(_accessor, _env);
                                alertData.OTP = requestModel.otp;
        ...
        public IActionResult DoUPIPaymentB2C([FromBody] ReqSendMoney reqSendMoney)
        {
            var wtw = new WebWTWUserInfo
            {
                Statuscode = ErrorCodes.Minus1,
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                ...
                if (!appResp.IsPasswordExpired)
                {
                    var dmr = new SellerML(_accessor, _env, false);
                    return Json(dmr.DoUPIPayment(reqSendMoney, paymentmode.UserID));
                    //  return Json(info);
        ...
        public async Task<IActionResult> B2COpDetail([FromBody] AppSessionReq data)
        {
            ...
            if (appResp.Statuscode == ErrorCodes.One)
            {
                ...
                return Json(new webB2cres
                {
                    Statuscode = ErrorCodes.One,
                    Msg = ErrorCodes.SUCCESS,
                    OperatorOptional = b,
                    OperatorDetail = a,
                    IsLookUpFromAPI = appResp.IsLookUpFromAPI,
                    IsTakeCustomerNo = ApplicationSetting.IsTakeCustomerNo
                });
            }
            else
            {
        ...
        #region MemberShip TargetApi by rishabh 
        [HttpPost]//24
        public async Task<IActionResult> GetB2cTargetAchieved()
        {
            var appResp = new WebB2cTargetAchieved
            {
                Statuscode = ErrorCodes.Minus1,
                Msg = ErrorCodes.AuthError
            };
            ...
            {
                var readHeader = ReadHeader();
                HttpContext.Request.Headers.TryGetValue("IsTotal", out var IsTotal);
                var appRequest = new B2cAppTargetRequest
                {
                    APPID = readHeader.AppID ?? "",
                    LoginTypeID = LoginType.ApplicationUser,
                    UserID = readHeader.UserID,
                    ...
                    Version = readHeader.Version ?? "",
                    Session = readHeader.Session,
                    IsTotal = Convert.ToBoolean(IsTotal[0])
                };
                var appResponse = appML.CheckWebAppSession(appRequest);
                appResp.Statuscode = appResponse.Statuscode;
                appResp.Msg = appResponse.Msg;
                ...
                if (appResp.Statuscode == ErrorCodes.One)
                {
                    ...
                    {
                        CommonInt = appRequest.UserID,
                        IsListType = appRequest.IsTotal
                    };
                    var res = await memberML.GetB2cTargetAchieved(req);
                    appResp.B2cTargetAchieved = res.ToList();
                }
            }
            catch (Exception ex) { }
            return Json(appResp);
