...
namespace RoundpayFinTech.Controllers
{
    public partial class AdminController
    {
        ...
        public IActionResult _BankMasterCU(int ID)
        {
            ...
            var smartML = new SmartCollectML(_accessor, _env);
            var collectTypes = smartML.GetSmartCollectType();
            bankMaster.smartCollectList = new SelectList(collectTypes, "Id", "SmartCollectTypeName");
        ...
        #region SmartCollectMaster
        [Route("SmartCollectType")]
        public IActionResult SmartCollectType()
        {
            if (_lr.RoleID == Role.Admin && _lr.LoginTypeID == LoginType.ApplicationUser)
                return View();
            return BadRequest();
        }
        [Route("GetSmartCollectType")]
        public IActionResult _GetSmartCollectType()
        {
            if (_lr.RoleID == Role.Admin && _lr.LoginTypeID == LoginType.ApplicationUser)
            {
                var smartML = new SmartCollectML(_accessor, _env);
                var res = smartML.GetSmartCollectType();
                return PartialView("partial/_GetSmartCollectType", res);
            }
            return BadRequest();
        }
        [HttpPost("changeGetSmartCollectType")]
        public IActionResult ChangeGetSmartCollectTypeStatus(SmartCollect smartCollect)
        {
            if (_lr.RoleID == Role.Admin && _lr.LoginTypeID == LoginType.ApplicationUser)
            {
                var smartML = new SmartCollectML(_accessor, _env);
                var res = smartML.ChangeSmartCollectTypeStatus(smartCollect);
                return Json(res);
            }
            return BadRequest();
        }
