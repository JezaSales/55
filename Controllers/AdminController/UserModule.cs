...
namespace RoundpayFinTech.Controllers
{
    public partial class AdminController
    {
        ...
        public IActionResult CreateUser(string m)
        {
            ...
            var empML = new EmpML(_accessor, _env);
            var emps = empML.GetEmployee(new CommonFilter
            {
                TopRows=200,
                btnID=1
            });
            ...
            {
                if (emps.Employees.Count > 0) {
                    res.EmployeeSelect = new SelectList(emps.Employees, "EmpID", "Name");
                }
            }
            //{
        ...
        [HttpPost]
        [Route("GetChangeIntro")]
        public IActionResult GetChangeIntro()
        {
            return PartialView("Partial/_ChangeIntro");
        }
        [HttpPost]
        [Route("UpdateIntro")]
        public IActionResult UpdateIntro(int UserID, string IntroMob)
        {
            return Json(uow.adminML.UpdateChangeIntro(UserID, IntroMob, _lr.UserID, _lr.LoginTypeID));
        }
        [HttpPost]
        [Route("UpdateRole")]
