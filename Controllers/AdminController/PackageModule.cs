...
namespace RoundpayFinTech.Controllers
{
    public partial class AdminController
    {
        ...
        public IActionResult _GetSellerAvailablePackages(int Id)
        {
            if (ApplicationSetting.IsPackageAllowed)
            {
                IUserML userML = new UserML(_lr);
        ...
        [HttpPost]
        [Route("Seller-Additional-Packages")]
        public IActionResult _GetAdditionalPackages(bool IsAddService, int opTypeid = 0)
        {
            var userML = new UserML(_accessor, _env);
            var res = userML.GetAddServiceSts(_lr.LoginTypeID, _lr.UserID, _lr.OutletID, opTypeid);
