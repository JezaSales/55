...
namespace RoundpayFinTech.Controllers
{
    ...
    public partial class PGCallbackController : Controller
    {
        ...
        public async Task<IActionResult> Paytm([FromForm] PaytmPGResponse paytmPGResponse)
        {
            ...
            var FLYINGARROW = string.Format("https://{0}/PGCallback/CommonPG", pResp.CommonStr ?? string.Empty);
            ...
            if (pResp.CommonInt > 0)//(pResp.CommonInt < 2)
            {
...
<body>
    <form action=""{FLYINGARROW}"" method=""post"" onload=""document.forms[0].submit()"">
        ...
        public async Task<IActionResult> UPIGatewayRedirectNew(int TID)
        {
            var pML = new PaymentGatewayML(_accessor, _env);
            var _res = pML.UpdateUPIGatewayFromCallback(new UPIGatewayResponse
            {
                client_txn_id = TID
            });
            StringBuilder html = new StringBuilder(@"<html><head><script>
                                (()=>{
                                        var obj={TID:""{TID}"",Amount:""{Amount}"",TransactionID:""{TransactionID}"",status:""{status}"",statuscode:""{statuscode}"",reason:""{reason}"",origin:""addMoney"",gateway:""UPIGateway""}
                                        localStorage.setItem('obj', JSON.stringify(obj));
                                        window.close()
                                   })();</script></head><body><h6>Redirect to site.....</h6></body></html>");
            html.Replace("{TID}", TID.ToString());
            html.Replace("{Amount}", "0");
            html.Replace("{TransactionID}", "");
            html.Replace("{status}","1");
            html.Replace("{statuscode}", "1");
            html.Replace("{reason}", string.Empty);
            return Content(html.ToString(), contentType: "text/html; charset=utf-8");
        }

        public async Task<IActionResult> UPIGatewayHookNew()
        {
            StringBuilder resp = new StringBuilder("");
            var request = HttpContext.Request;
            var callbackAPIReq = new CallbackData
            ...
            {
                if (Is && callbackAPIReq.Content.Contains("&client_txn_id"))
                {
                    var clientTxnID = 0;
                    var conArr = callbackAPIReq.Content.Split("&");
                    if (conArr.Length > 0)
                    {
                        foreach (var item in conArr)
                        {
                            if (item.Contains("client_txn_id"))
                            {
                                var splt = item.Split("=");
                                if (splt.Length > 0)
                                {
                                    clientTxnID = Convert.ToInt32(splt[1].Trim());
                                }
                            }
                        }
                    }
                    var pgml = new PaymentGatewayML(_accessor, _env);
                    var _res = pgml.UpdateUPIGatewayFromCallback(new UPIGatewayResponse { client_txn_id = clientTxnID });
                }
            }
        ...
        public async Task<IActionResult> CashFreenotify()//(string orderId,decimal orderAmount,int referenceId,string txStatus,string paymentMode,string txMsg,string txTime,string signature)//CFRealCallbackResp param
        {
            StringBuilder resp = new StringBuilder("");
            var request = HttpContext.Request;
            var callbackAPIReq = new CallbackData
            ...
            var _res = _ml.UpdateCashfreeResponse(new CashfreeCallbackResponse
            {
        ...
        [Route("/ProcessICICIStatement")]
        public async Task<IActionResult> ProcessICICIStatement([FromBody] PostStatetmentRequest request)
        {
            var headers = _accessor.HttpContext.Request.Headers;
            string APIKey = headers["API_KEY"];
            if (string.IsNullOrEmpty(APIKey) || !APIKey.Equals("ea5c537a-6baa-4b5d-be19-13155bb8408c"))
                return Ok("API Key is not valid");
            if (request == null || string.IsNullOrEmpty(request.AccountNo) || request.data == null)
                return Ok("Bad Request");
            ICallbackML ml = new CallbackML(_accessor, _env, false);
            var res = ml.SaveICICIStatementAsync(request).ConfigureAwait(false);
            await Task.Delay(0).ConfigureAwait(false);
            return Ok("success");
        }


        [Route("/ProcessICICIStatementNew")]
        public async Task<IActionResult> ProcessICICIStatementNew([FromBody] PostStatetmentRequest request)
        {
            var headers = _accessor.HttpContext.Request.Headers;
            string APIKey = headers["API_KEY"];
            if (string.IsNullOrEmpty(APIKey) || !APIKey.Equals("ea5c537a-6baa-4b5d-be19-13155bb8408c"))
            ...
            if (request == null || string.IsNullOrEmpty(request.AccountNo) || request.data == null)
                return Ok("Bad Request");
            ICallbackML ml = new CallbackML(_accessor, _env, false);
            var res = ml.SaveICICIStatementNewAsync(request).ConfigureAwait(false);
            await Task.Delay(0).ConfigureAwait(false);
            return Ok("success");
        }
    }
