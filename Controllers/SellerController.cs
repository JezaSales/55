...
namespace RoundpayFinTech.Controllers
{
    ...
    public partial class SellerController : Controller
    {
        ...
        public async Task<IActionResult> rech(int s)
        {
            ...
            if (servicesPM.Detail.Any(x => x.IsAdditionalServiceType))
            {
                var isPaid = servicesPM.Detail.Any(x => x.IsPaidAdditional);
                if (!isPaid)
                {
                    servicesPM.IsAdditionalServiceType = true;
                    servicesPM.IsPaidAdditional = false;
                }
            }
        ...
        [HttpPost]
        [Route("DeleteBene-p")]
        public IActionResult DeleteBeneP(int o, string MobileNo, string BeneID, string SID, string OTP,string RefID)
        {
            var dmr = new SellerML(_accessor, _env);
        ...
        public IActionResult RechSimpPlans(int o, int c)
        {
            if (ApplicationSetting.IsPlanServiceUpdated)
            {
                ...
                var data = plansAPIML.GetRNPRechargePlans(o, c, _lr.LoginTypeID, _lr.UserID);
            ...
            else
            {
                ...
                else if (ApplicationSetting.PlanType == PlanType.ROFFERCoPlan)
                {
                    var plansAPIML = new PlansAPIML(_accessor, _env);
                    var resp = plansAPIML.GetRoSimplePlan(c, o);
                    return PartialView("Partial/MPLAN/SimplePlan", resp);
                }
            }
        ...
        public IActionResult Roffer(int o, string a)
        {
            if (ApplicationSetting.IsRoffer)
            {
                ...
                else
                {
                    ...
                    {
                        // new api https://roffer.in/
                        var plansAPIML = new PlansAPIML(_accessor, _env);
                        var resp = plansAPIML.GetRRoffer(a, o);
                        return PartialView("Partial/MPLAN/Roffer", resp);
                    }
                }
            }
        ...
        public IActionResult DTHSimpPlan(int o)
        {
            ...
            {
                var plansAPIML = new PlansAPIML(_accessor, _env);
                var data = plansAPIML.GetRNPDTHPlans(o, _lr.LoginTypeID, _lr.UserID);
            ...
            else
            {
                ...
                {
                    var plansAPIML = new PlansAPIML(_accessor, _env);
                    var resp = plansAPIML.GetRoDTHSimplePlan(o);
                    return PartialView("Partial/MPLAN/DTHPlan", resp);
                }
                return BadRequest(new { Msg = "No setting found" });
            }
        ...
        public IActionResult DTHCustInfo(int o, string a)
        {
            if (ApplicationSetting.IsDTHInfo)
            {
                ...
                else
                {
                    ...
                    {
                        var plansAPIML = new PlansAPIML(_accessor, _env);
                        var resp = plansAPIML.GetRDTHCustInfo(a, o);
                        return PartialView("Partial/MPLAN/DTHCustInfo", resp);
                    }
                }
        ...
        public IActionResult DTHHeavyRefresh(int o, string a)
        {
            if (ApplicationSetting.IsHeavyRefresh)
            {
                ...
                else
                {
                    ...
                    {
                        var plansAPIML = new PlansAPIML(_accessor, _env);
                        var resp = plansAPIML.GetRoDTHHeavyRefresh(a, o);
                        return PartialView("Partial/MPLAN/DTHHeavyRefresh", resp);
                    }
                }
        ...
        public IActionResult GIRedirect(string code, string prodid, string tranid, int outlet)
        {
            ...
            if (code == APICode.GIBL)
            {
                ...
                string userdata = Convert.ToBase64String(data);
        ...
        public IActionResult _GetAffilietedItems(int VendorID)
        {
           ...
           // var res = ml.GetAllAfItems(VendorID, _lr.UserID, _lr.LoginTypeID);
            var res = ml.GetAllAffiliatedItems(_lr.UserID, _lr.LoginTypeID).Result;
            return PartialView("Affiliates/Partial/_GetAffilietedItems", res);
        }

        [HttpPost]
        [Route("GetAffilieted")]
        public IActionResult GetAffilieted(int ID)
        {
            IAffiliationML ml = new AffiliationML(_accessor, _env);
            var res = ml.GetAfItems(ID, _lr.UserID, _lr.LoginTypeID);
            return Json(res);
        }


        [Route("GetAffilietedproduct")]
        public IActionResult GetAffilietedproduct(int OID)
        {
            return View();
        }

        [HttpPost]
        [Route("_GetAffilietedproduct")]
        public IActionResult _GetAffilietedproduct(int OID)
        {
            IAffiliationML ml = new AffiliationML(_accessor, _env);
            var res = ml.GetAffilietedproduct(OID);
            //res = res.Where(x => x.IsActive);
            return PartialView("Affiliates/Partial/_GetAffilietedproduct", res);
        }

        [HttpPost]
        [Route("_GetAffilietedVproduct")]
        public IActionResult _GetAffilietedVproduct(int VID)
        {
            IAffiliationML ml = new AffiliationML(_accessor, _env);
            var res = ml.GetAffilietedVproduct(VID);
            //res = res.Where(x => x.IsActive);
            return PartialView("Affiliates/Partial/_GetAffilietedproduct", res);
        }
        ...
        public IActionResult GetAfCategoriesWithProduct(int categoryId = 0)
        {
            IAffiliationML ml = new AffiliationML(_accessor, _env);
            var res = ml.GetAfCategoriesWithProduct(_lr.UserID, _lr.LoginTypeID, categoryId);
            return Json(res);
        }


        [HttpPost("GetAfVendorsWithProduct")]
        public IActionResult GetAfVendorsWithProduct(int vendorId = 0)
        {
            IAffiliationML ml = new AffiliationML(_accessor, _env);
            var res = ml.GetAfVendorsWithProduct(_lr.UserID, _lr.LoginTypeID, vendorId);
            return Json(res);
        }
        ...
        [HttpPost("GenrateAfLink")]
        public async Task<IActionResult> GenrateAfLink(int productId)
        {
            IAffiliationML ml = new AffiliationML(_accessor, _env);
            var res = await ml.GenrateAfLink(productId, _lr.UserID);
            return Json(res);
        }
    }
}